...
import { ArrowUpRight, Clock, User, Video, Zap } from "lucide-react";
import type { Route } from "next";
import Image from "next/image";
import Link from "next/link";

interface ExtendedStudentSession extends StudentSession {
	xpReward?: number;
}

interface UpcomingSessionsProps {
	sessions?: ExtendedStudentSession[];
}
			...
			<div className='space-y-3'>
				{sessions.length === 0 ? (
					...
					sessions.map((sessionItem) => {
						const xpReward = sessionItem.xpReward ?? 50; // Default XP per completed lesson
								...
								<div className='flex items-start gap-3.5'>
									...
									<div className='space-y-1'>
										<div className='flex flex-wrap items-center gap-2'>
											...
											<span className='inline-flex items-center gap-0.5 rounded-md bg-amber-500/10 px-2 py-0.5 text-xs font-semibold text-amber-600 dark:text-amber-400'>
												<Zap size={12} className='fill-amber-600 dark:fill-amber-400' />+{xpReward}{" "}
												XP
											</span>
